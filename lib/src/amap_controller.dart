...
class AMapController {
  ...
  void _listen<T>(Stream<T> stream, void Function(T event) onData) {
    _streamSubscriptions.add(stream.listen(onData));
  }

  void _connectStreams(int mapId) {
    if (_mapState.widget.onLocationChanged != null) {
      _listen<LocationChangedEvent>(
        _methodChannel.onLocationChanged(mapId: mapId),
        (LocationChangedEvent e) => _mapState.widget.onLocationChanged!(e.value),
      );
    }

    if (_mapState.widget.onCameraMove != null) {
      _listen<CameraPositionMoveEvent>(
        _methodChannel.onCameraMove(mapId: mapId),
        (CameraPositionMoveEvent e) => _mapState.widget.onCameraMove!(e.value),
      );
    }
    if (_mapState.widget.onCameraMoveEnd != null) {
      _listen<CameraPositionMoveEndEvent>(
        _methodChannel.onCameraMoveEnd(mapId: mapId),
    ...
    if (_mapState.widget.onTap != null) {
      _listen<MapTapEvent>(
        _methodChannel.onMapTap(mapId: mapId),
        (MapTapEvent e) => _mapState.widget.onTap!(e.value),
      );
    }
    if (_mapState.widget.onLongPress != null) {
      _listen<MapLongPressEvent>(
        _methodChannel.onMapLongPress(mapId: mapId),
        (MapLongPressEvent e) => _mapState.widget.onLongPress!(e.value),
      );
    }

    if (_mapState.widget.onPoiTouched != null) {
      _listen<MapPoiTouchEvent>(
        _methodChannel.onPoiTouched(mapId: mapId),
        (MapPoiTouchEvent e) => _mapState.widget.onPoiTouched!(e.value),
      );
    }

    _listen<MarkerTapEvent>(
      _methodChannel.onMarkerTap(mapId: mapId),
      (MarkerTapEvent e) => _mapState.onMarkerTap(e.value),
    );

    _listen<MarkerDragEndEvent>(
      _methodChannel.onMarkerDragEnd(mapId: mapId),
      (MarkerDragEndEvent e) => _mapState.onMarkerDragEnd(e.value, e.position),
    );

    _listen<PolylineTapEvent>(
      _methodChannel.onPolylineTap(mapId: mapId),
      (PolylineTapEvent e) => _mapState.onPolylineTap(e.value),
    );
  }
