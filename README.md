# amap_flutter_map_plus

This repository is a maintained fork used by Squady.

Why this fork exists:
- The upstream `amap_flutter_map` package on `pub.dev` is outdated for the current Android Gradle Plugin / Flutter toolchain.
- Our app depends on fixes such as Android `namespace` support and newer Flutter compatibility.
- The original `pub.dev` package does not expose a public source repository, so this fork is the maintainable source of truth for our app.

Current dependency boundary:
- This package is maintained here as a standalone repository.
- `amap_flutter_base_plus` is still consumed as an external package and has not been vendored into this repository yet.
